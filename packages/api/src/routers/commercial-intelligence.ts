...
export const commercialIntelligenceRouter = router({
  ...
  objections: permittedProcedure(["leads:read"]).input(commercialDateRangeInput).query(({ ctx, input }) => getObjectionMotivationIntelligence({ from: day(input.from), to: day(input.to, true), actorId: ctx.permissions.includes("*") ? null : ctx.session.user.id })),
